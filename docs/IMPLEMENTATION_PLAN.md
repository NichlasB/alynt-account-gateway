...
- Current phase: Phase 2 Increment 8 settings-page decomposition is in progress from the released v1.1.20 baseline.
- Target path: `C:\Development\WordPress\Plugins\alynt-account-gateway`
- Plugin status: v1.1.20 is the current public baseline.
- Frontend output default: Disabled
- Distribution: Alynt-distributed plugin with GitHub updater compatibility
- Next roadmap: Complete the settings-page decomposition, full quality gates, exact-package inspection, and LocalWP Plugin Tester acceptance before presenting v1.1.21 for separate release approval. Inactive-account integration remains deferred until an authoritative status source exists.
...
- [x] Publish v1.1.16 only after separate explicit release approval.
...
- Release approval was granted and [GitHub v1.1.16](https://github.com/NichlasB/alynt-account-gateway/releases/tag/v1.1.16) was published from merge commit `6274b0b`; Build Release run `29727428023` completed successfully.
- The public release ZIP contains `51` runtime files and `45` syntax-clean PHP files, contains no development files or backslash archive paths, and has SHA-256 `793B3C8CBF96583D2745A4639331C92AC23D14F00FA52C6907F0EF8AF8BCD49D`.
...
- [x] Lock the v1.1.17 baseline at `396` tests and `2,743` assertions.
- [x] Lock exact SHA-256 fingerprints for the ordered tabs, complete schema metadata, and derived defaults.
- [x] Keep `ALYNT_AG_Settings_Schema` as the public static facade with its established twelve public methods.
- [x] Extract ordered core, security/email, and account/data field definitions into focused providers behind `ALYNT_AG_Settings_Definition`.
- [x] Extract default derivation, tab defaults, tab restoration, and stored-value resolution into `ALYNT_AG_Settings_Defaults`.
- [x] Extract schema filtering and type-specific sanitization into `ALYNT_AG_Settings_Sanitizer`.
- [x] Preserve tab order, field order, field metadata, generated bypass defaults, stored-value merging, import/export behavior, and sanitized output.
- [x] Load all collaborators before the settings facade in production and test bootstraps.
...
- The exact tab, schema, and defaults fingerprints remain identical to the v1.1.17 baseline.
- Frontend/admin build and POT generation pass; the POT remains stable at `1,104` strings and reports project version `1.1.18`.
- Full PHPCS, project-wide PHP syntax, and source JavaScript syntax checks pass.
...
- [ ] Inspect the exact v1.1.21 release-style package.
- [ ] Install the exact candidate on LocalWP Plugin Tester and verify all settings tabs, security operations, email/webhook tools, diagnostics, preview, import/export, and responsive behavior without changing persistent settings or activation state.
...
- The suite increased from `420` tests / `2,911` assertions to `424` tests / `3,121` assertions.
...
- Novamira MCP remains unavailable in this session. LocalWP candidate acceptance will use the approved WP-CLI, MariaDB, and Playwright workflow on `plugin-tester local-only`.
...
- A fresh production restore point was created at `/var/www/classes.internationalschoolofthehealingarts.com/restores/alynt-account-gateway-v1.1.11-preupdate-20260719-142733`. Its database export, exact active-plugin snapshot, v1.1.10 Account Gateway archive, v1.1.5 updater archive, ownership, permissions, and checksums passed.
...
- [x] Preview all five configured production account email templates.
- [x] Confirm desktop logo sizing, responsive typography, CTA rendering, token replacement, and overflow behavior.
- [x] Reproduce narrow-screen overflow in templates containing long fallback action URLs.
- [x] Isolate the intrinsic-width expansion to the unwrapped fallback URL paragraph.
- [x] Add email-client-safe inline wrapping rules without changing the action button or plain-text fallback.
...
- At `390px`, registration confirmation, password reset, account-created welcome, and email-change confirmation previews expanded beyond the viewport because their long fallback action URL had no inline wrapping behavior. Password changed did not overflow because it has no action URL.
...
- [x] Publish and updater-verify the release after explicit approval.
...
- Released as [`v1.1.8`](https://github.com/NichlasB/alynt-account-gateway/releases/tag/v1.1.8); Build Release workflow run `29685145001` passed.
...
#### Immediate Increment - Account Details Summary
...
- [x] The module shows customer-facing name, email, and customer-since values without username fallback.
- [x] Incomplete names produce a calm review state and a native Account Details action.
- [x] Hiding Account Details removes the module and navigation affordance while leaving the direct WooCommerce endpoint available.
- [x] Values wrap without horizontal overflow down to 320px and all action text remains at least 16px.
...
- [x] Wait for explicit release approval before merge, tag, push, or publication.
- [x] Verify the public GitHub asset and Alynt Plugin Updater path after publication.
...
- Published GitHub release [`v1.0.0`](https://github.com/NichlasB/alynt-account-gateway/releases/tag/v1.0.0) from merge commit `a144e2a`; Build Release workflow run `29589749955` completed successfully.
- Downloaded and inspected public asset `alynt-account-gateway-v1.0.0.zip`: 45 runtime files, one expected plugin root, forward-slash ZIP entries, zero development/source files, aligned `1.0.0` metadata, exactly one updater header, and SHA-256 `AF0AB93375F3CF3C3266AFFB7ECE4905DD97F9C2082B1E8D2EDF3C3C172D688F`.
- Public updater verification passed on `hbf-staging`: the site was temporarily restored to public `v0.1.120`, Alynt Plugin Updater detected `0.1.120 -> 1.0.0`, WordPress downloaded `https://github.com/NichlasB/alynt-account-gateway/releases/download/v1.0.0/alynt-account-gateway-v1.0.0.zip`, and final state reports active `1.0.0`, 45 runtime files, unchanged settings and active-plugin hashes, `/account` HTTP 200, and no remaining ACG update offer.
...
Phase 6 compatibility work is complete. The target-current `hbf-staging` environment inventory records WordPress `7.0.1`, PHP `8.2.32`, WooCommerce `10.9.4`, Blocksy child/Blocksy, `en_US` LTR, Redis/FluentSMTP/security stack, PayPal/NMI gateways, and USPS shipping in `docs/V1_READINESS_PLAN.md`. The mobile/desktop route matrix for login, registration, lost password, set password, logout, dashboard, and WooCommerce account routes passed on the target-current staging stack. Frontend 800px boundary behavior and admin tab wrapping passed; the keyboard/focus/live-region/password-control evidence passed; `P6-001` narrow Webhooks table overflow was closed by public `v0.1.117`, installed through Alynt Plugin Updater and retested at `480px`. `P6-002` registration Terms and Privacy Policy link focus styling was closed by public `v0.1.118`, installed through Alynt Plugin Updater and retested at `390x844` and `1440x1000`. `P6-003` registration Turnstile 320px overflow was closed by public `v0.1.119`, installed through Alynt Plugin Updater and retested at `320x1000`; clean reduced-motion retesting also passed. The request/console/log slice found no Account Gateway remote font or tracking dependency, no plugin-caused browser or PHP errors, an empty plugin diagnostics log, and only documented external noise from Independent Analytics, site-stack third-party assets, browser-local filtering, and configured Cloudflare Turnstile. The multilingual/RTL slice used temporary QA translations and a query-scoped locale switcher to verify Spanish LTR frontend/admin translation loading and Arabic RTL frontend/admin rendering, including LTR email fields and zero frontend overflow, then removed all QA files and the disposable administrator. The minimum-version/default-theme slice passed PHPCompatibilityWP for PHP `7.4-`, source/API review for the WordPress `6.0` floor, and a LocalWP Plugin Tester smoke on Twenty Twenty-Five with the current `0.1.119` runtime files.
