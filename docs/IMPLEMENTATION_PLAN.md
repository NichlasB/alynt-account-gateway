...
Phase 6 compatibility work has started with a target-current `hbf-staging` environment inventory: WordPress `7.0.1`, PHP `8.2.32`, WooCommerce `10.9.4`, Blocksy child/Blocksy, `en_US` LTR, Redis/FluentSMTP/security stack, PayPal/NMI gateways, and USPS shipping are recorded in `docs/V1_READINESS_PLAN.md`. The mobile/desktop route matrix for login, registration, lost password, set password, logout, dashboard, and WooCommerce account routes passed on the target-current staging stack. Frontend 800px boundary behavior and admin tab wrapping passed; `P6-001` narrow Webhooks table overflow was closed by public `v0.1.117`, installed through Alynt Plugin Updater and retested at `480px`. The keyboard/focus/live-region pass found `P6-002`, where registration Terms and Privacy Policy links used only browser-default focus styling; a scoped frontend CSS candidate fix is prepared and browser-injection verified, pending release/updater retest. Minimum-supported-version, default-theme, remaining accessibility coverage, RTL/multilingual, request, console, and log coverage remain in the readiness plan.
