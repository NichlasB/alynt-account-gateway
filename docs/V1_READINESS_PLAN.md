...
| Publication state | `v0.1.106` committed, tagged, pushed, published, public ZIP inspected, installed on `hbf-staging` through Alynt Plugin Updater, and all five account-email templates resent through the site mail path | Mailbox confirmation pending |
...
| `P1-005` | 1 / 3 | Low | Account-email body text felt too small during real mailbox review after logo correction. | Product owner | Confirm the resent `v0.1.106` mailbox rendering after responsive body text sizing was released and installed. | Local candidate tests, package inspection, updater install `0.1.105 -> 0.1.106`, installed-file verification, and five resent account-email templates | Open mailbox confirmation |
...
- Finding `P1-005` records that the real mailbox render now has acceptable logo sizing but the email body copy should be more readable. Public `v0.1.106` adds responsive body text sizing with a 16px mobile/default fallback, 18px tablet sizing, and 20px desktop sizing, plus focused regression coverage, has been installed on `hbf-staging` through Alynt Plugin Updater, and resent all five account-email templates. Mailbox-side confirmation remains pending.
