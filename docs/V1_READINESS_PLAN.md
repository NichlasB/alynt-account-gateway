...
- Current phase: Phase 1 is active. Public `v0.1.106` is updater-verified, representative HBF configuration is saved with all public-output switches disabled, visual preview evidence is captured, legal destinations are configured for staging, and email test sends are accepted by the site mail path. `P1-004` email-logo correction is mailbox-confirmed. `P1-005` email body typography remains open because `v0.1.106` did not visibly change the real mailbox body copy; corrected inline-sizing candidate `v0.1.107`, provider/webhook configuration, live provider/webhook acceptance, and route handover remain gated.
...
| `P1-005` | 1 / 3 | Low | Account-email body text felt too small during real mailbox review after logo correction. `v0.1.106` did not visibly change the real mailbox because the client ignored the wrapper/media-query approach. | Product owner | Prepare `v0.1.107` with inline paragraph and list-item body sizing, then release, updater-install, resend, and confirm mailbox rendering. | Focused and full candidate tests, PHPCS, syntax sweep, build, POT regeneration, and cleaned-up inline-sizing implementation | Open corrective release |
