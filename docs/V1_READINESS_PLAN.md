...
| Remaining Turnstile coverage | Invalid-token server-side failure was verified earlier. Token expiry, replay, provider outage/timeout, and Turnstile-only success remain for later Phase 4 slices. | Pending |
...
- The Phase 4 Turnstile browser-success slice passed on `v0.1.113`: the public registration screen produced a real Cloudflare Turnstile response token in the browser, a disposable registration submitted successfully to `registration_sent=1`, plugin-owned verification logs recorded Turnstile `passed` with `blocked=0` and Reoon `valid` with `blocked=0`, no WordPress user was created before confirmation/password completion, and all disposable pending, consent, verification, and helper artifacts were removed. Token expiry, replay, provider outage/timeout, and Turnstile-only success remain for later Phase 4 slices.
