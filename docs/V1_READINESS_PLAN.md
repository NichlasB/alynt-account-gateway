...
- Current phase: Phase 1 is active. Public `v0.1.104` is updater-verified, representative HBF configuration is saved with all public-output switches disabled, and visual preview evidence is captured. Legal destinations, real email delivery, approved integration credentials, and route handover remain gated.
...
| Legal destinations | Saved settings use `/terms/` and `/privacy/`, which do not resolve directly. WordPress redirects those paths to published custom `legal` posts at `/legal/terms/` and `/legal/privacy/`, but anonymous requests are still intercepted by the incumbent login stack. | Blocking before registration |
...
Representative configuration is complete for the inputs currently available. Frontend Output remains disabled. Before route handover, the legal-page decision, a site-owned email test recipient, and integration credential decisions remain open.
...
| `P1-002` | 1 | Medium | The configured `/terms/` and `/privacy/` paths do not resolve directly, and their redirected `/legal/terms/` and `/legal/privacy/` destinations are not anonymously reachable while Force Login owns public access. | Site owner | Keep public registration disabled until approved legal destinations are configured and both links are reachable for anonymous registration visitors. | Published-page, anonymous HTTP, and frontend-link verification | Open configuration blocker |
...
- Finding `P1-002` records that the saved `/terms/` and `/privacy/` paths do not resolve directly, while the redirected `/legal/terms/` and `/legal/privacy/` custom `legal` posts are published but not anonymously reachable under the incumbent Force Login behavior. A site-owned email test recipient and approved provider/webhook credentials or explicit disable decisions are also still required for later acceptance.
