...
- Current phase: Phase 6 is active. Public `v0.1.117` is updater-verified on `hbf-staging`, Phase 5 dashboard/WooCommerce acceptance is complete, and Phase 6 target-current environment inventory, mobile/desktop route-matrix testing, 800px frontend boundary testing, and narrow admin layout testing passed on WordPress `7.0.1`, PHP `8.2.32`, WooCommerce `10.9.4`, Blocksy child theme, `en_US` LTR, Redis/FluentSMTP/security plugin stack, PayPal/NMI gateways, and USPS shipping. Minimum-version, default-theme, accessibility, RTL/multilingual, third-party-request, and error-log/console coverage remain open.
- Product baseline: `v0.1.117`, released, public-asset verified, and updater-verified on production-like staging.
- Release goal: `v1.0.0`.
...
| Error association and live regions | Login, registration, lost-password, and set-password error states rendered alert/live-region notices. Login, registration, and lost-password invalid fields were associated with error text through `aria-describedby`; set-password rendered the alert state and requires packaged retest after the focus correction. | Partially passed |
...
| Candidate correction | Local source CSS now includes `.agw-checkbox a:focus` and `.agw-checkbox a:focus-visible` in the frontend focus selector groups, including the forced-colors block. Browser injection of the exact candidate rules on `hbf-staging` at `390x844` changed both agreement links to a solid visible gateway focus outline. | Pending release |
| Cleanup and restore | Original settings were restored; disposable users `9288` and `9289`, temporary remote helpers, local helper scripts, and snapshot option were removed. Final cleanup returned plugin `0.1.117`, zero matching accessibility QA users, no snapshot option, and no helper leftovers. | Completed |
...
| `P6-002` | 6 | Low | Registration Terms and Privacy Policy links inside the agreement checkbox used only the browser default dotted focus outline instead of the plugin's visible gateway focus treatment. | Product owner | Candidate fix prepared for the next patch release: include `.agw-checkbox a` in normal, focus-visible, and forced-colors frontend focus selector groups. | Initial registration focus inspection at `390x844`; local source patch; build, PHPCS, PHPUnit, and diff checks passed; browser injection of candidate rules made both links render a solid visible focus outline | Open pending release |
