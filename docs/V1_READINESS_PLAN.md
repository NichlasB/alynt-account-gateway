...
| Local corrective candidate | Local `v0.1.117` contains a scoped `@media (max-width: 640px)` rule that makes plugin admin `widefat` tables block-level and internally scrollable. Injecting the exact rule into the loaded staging Webhooks page removed the page-level overflow. `npm run build`, `npm run lint`, `npm test`, `npm run make-pot`, and `git diff --check` passed locally. | Prepared; release approval required |
...
| `P6-001` | 6 | Low | The narrow Webhooks settings tab could create page-level horizontal overflow at `480px` because the plugin-owned `widefat` webhook log table exceeded the viewport. | Product owner | Local `v0.1.117` candidate contains a scoped small-width admin table overflow fix; release and updater retest required before closure. | Initial `480px` Webhooks tab browser check; offender inspection identified the `widefat` table; exact-rule browser injection removed page-level overflow; local build, PHPCS, PHPUnit, POT, and diff checks passed | Open |
