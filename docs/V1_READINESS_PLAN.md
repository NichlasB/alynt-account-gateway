...
- Current phase: `v1.1.4` is released, public-asset inspected, and installed on LocalWP `plugin-tester` and production-like `hbf-staging`.
- Product baseline: `v1.1.4`, released, public-asset verified, and updater-verified with Alynt Plugin Updater on LocalWP Plugin Tester and `hbf-staging`.
- Release goal: `v1.0.0`.
...
### v1.1.4 Available Downloads Dashboard Module

| Item | Value | Evidence | Status |
| --- | --- | --- | --- |
| Release | [`v1.1.4`](https://github.com/NichlasB/alynt-account-gateway/releases/tag/v1.1.4) | GitHub release from commit `93411d2` | Published |
| Build Release workflow | Run `29646939926` | GitHub Actions | Passed |
| Public asset | `alynt-account-gateway-v1.1.4.zip`; `47` runtime files; one plugin root; zero development-file or backslash-path hits; aligned `1.1.4` metadata and Available Downloads feature markers; all `41` packaged PHP files syntax-clean | Public asset inspection | Verified |
| Public asset SHA-256 | `3F40999D9BB927DD6C61036BC13E527DDBB8DC52DEDEACAC418734C77FF59991` | GitHub digest and local hash of downloaded public release asset | Recorded |
| Product change | The WooCommerce dashboard now shows a bounded, read-only three-file Available Downloads preview with finite/unlimited and expiry states, calm empty guidance, and delegated WooCommerce-owned actions. | `CHANGELOG.md`, `readme.txt`, packaged runtime, and acceptance evidence | Completed |
| Plugin Tester updater | Alynt Plugin Updater force-fresh offered the exact public asset and WordPress installed `1.1.3 -> 1.1.4`; active position, active-plugin hash, and settings hash remained exact, all 47 files byte-matched the public ZIP, feature markers were present, no update remained, and `/login/` returned `200`. | Updater probe, filesystem hash comparison, and HTTP smoke | Passed |
| Staging updater | `hbf-staging` installed the exact public updater offer `1.1.3 -> 1.1.4` through the site-owner WordPress upgrader. Settings remained exact. The upgrader moved Account Gateway from active position `7` to `8`; guarded reconciliation restored exact position `7` and the recorded active-plugin hash. Final state retained 47 runtime files, matching public main-file digest, normal site ownership, no update offer, and `/login/` returned `200`. | WP-CLI, filesystem digest, HTTP smoke, and Site Operations evidence | Passed with positional reconciliation |
| Cleanup | Temporary updater helpers, reconciliation scripts, release-note scratch file, ZIP extracts, and remote `/tmp` artifacts were removed after evidence capture. | Local and remote cleanup verification | Completed |
...
- [x] Verify disabling or uninstalling the plugin does not remove WordPress users, WooCommerce orders, or unrelated media.
- [x] Verify uninstall removes only documented plugin-owned options, tables, scheduled hooks, and transient data.
- [x] Prepare GDPR-facing site-owner documentation for plugin-owned records, retention, third-party boundaries, and data-subject request support.
- [ ] Review GDPR-facing documentation with a qualified adviser where required.
...
- Disable/uninstall lifecycle evidence on `hbf-staging` used disposable run `20260717140847`. The preflight found existing plugin-owned evidence rows in webhook, verification, and audit tables, so the helper snapshotted and hash-restored those rows instead of treating the tables as disposable. Deactivation removed the `alynt_ag_retention_cleanup` schedule while preserving settings, database version, plugin tables, existing table rows, and disposable WordPress user, WooCommerce order, and attachment fixtures. The controlled uninstall-script pass removed `alynt_ag_settings`, `alynt_ag_db_version`, the retention schedule, all six documented plugin-owned tables, and `alynt_ag_rl_*` / `alynt_ag_rl_meta_*` transient option rows. It preserved the disposable WordPress user, WooCommerce order, attachment, unrelated control option, and active-plugin option. Recovery recreated the plugin tables, restored the exact settings hash, restored the exact plugin-table row snapshot hash, restored the retention schedule, and preserved the active-plugin hash. Cleanup removed the disposable user, order, attachment, control option, rate-limit transient rows, local helper, and remote helper files; final health confirmed Alynt Account Gateway active at `0.1.120` with `/account` returning HTTP 200.
- GDPR-facing documentation preparation added `docs/PRIVACY_AND_GDPR.md` and linked it from the README, `readme.txt`, and settings reference. The guide summarizes role boundaries, plugin-owned records, default privacy-safe posture, privacy notice prompts, data-subject request support, retention defaults, third-party sharing review points, and operational launch checks. It cites official European Commission and ICO references, and it explicitly states that site owners remain responsible for privacy notice wording, lawful-basis decisions, processor contracts, retention policy, international-transfer review, and qualified legal review where required.
