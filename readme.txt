...
* Fix randomized WooCommerce account-data test isolation so the v1.1.33 dashboard notice-button contrast fix can ship through a clean Quality workflow.

= 1.1.33 =

* Fix WooCommerce notice buttons inside the branded dashboard so their text keeps the configured button contrast instead of inheriting notice-link styling.

= 1.1.32 =

* Add a default-off Saved Payment Methods setting and replace technical payment/account diagnostics with customer-facing dashboard guidance.
...
* Use a neutral default login instruction for new installations. Existing saved screen copy is unchanged.

= 1.1.26 =

* Fix standalone branded gateway routes so WordPress emits one accurate document title and normal page metadata instead of a second 404 title fragment.

= 1.1.25 =

* Add native-reviewed Polish (`pl_PL`) account, authentication, dashboard, email, and operator-interface translations.
...
* Preserve manual-order customer payment handoffs by allowing authenticated WordPress `admin-post.php` action dispatchers while retaining the customer wp-admin screen restriction.
...
* Extract registration request, protection, activity, persistence, confirmation, completion, delivery, and credential concerns behind the existing registration service facade.
* Extract authentication request, activity, public-message, password-reset, and role-aware redirect concerns behind the existing authentication service facade.
* Preserve established public methods, constructor compatibility, subclass overrides, request behavior, account creation, password recovery, and redirects.
* Add public-API, collaborator-delegation, override-path, loader-order, and structural line-limit regression coverage.

= 1.1.18 =

* Extract ordered settings definitions, default derivation, stored-value access, and type-specific sanitization behind the existing settings schema facade.
* Preserve the established public static settings API, schema order and metadata, default values, import/export behavior, and sanitized output.
* Add exact definition/default fingerprints, collaborator delegation coverage, and structural line-limit checks.

= 1.1.17 =

* Extract the dashboard navigation, WooCommerce endpoint, commerce overview, and account-summary renderers behind the existing frontend dashboard facade.
* Preserve the established dashboard constructor contract, public rendering methods, WooCommerce delegation, markup classes, and frontend behavior.
* Add regression coverage for collaborator delegation, exact endpoint context, and dashboard renderer line limits.

= 1.1.16 =

* Extract the WordPress personal-data exporter and eraser into focused collaborators behind the existing privacy-service callbacks.
* Preserve exporter records, eraser deletion paths, consent handling, and public callback signatures.
* Add regression coverage for facade callback ownership and collaborator delegation.
